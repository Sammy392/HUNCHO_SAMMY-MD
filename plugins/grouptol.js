...
async (cmd, mek, m, {
    ...
    try {
        ...
        for (let participant of nonAdminParticipants) {
            try {
                await cmd.groupParticipantsUpdate(from, [participant.id], "remove");
...
async (cmd, mek, m, {
    ...
    try {
        ...
        const allParticipants = groupMetadata.participants;
        const adminParticipants = allParticipants.filter(member => groupAdmins.includes(member.id) && member.id !== cmd.user.id && member.id !== `${botOwner}@s.whatsapp.net`);
...
async (cmd, mek, m, {
    ...
    try {
        ...
        const participantsToRemove = allParticipants.filter(
            participant => participant.id !== cmd.user.id && participant.id !== `${botOwner}@s.whatsapp.net`
        );
